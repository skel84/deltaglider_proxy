...
mod tests {
    ...
    #[test]
    fn diff_redacted_secret_survives_updates_and_explicit_rotation() {
        let mut yaml = DeclarativeIam {
            users: vec![DeclarativeUser {
                secret_access_key: String::new(),
                enabled: false,
                ..yu("alice", "AKIA1")
        ...
        let current = CurrentIam {
            users: vec![IamUser {
                secret_access_key: "existing-dummy-secret".into(),
                ..db_user(1, "alice", "AKIA1")
            }],
            ..empty_db()
        };
        let diff = diff_iam(&yaml, &current).unwrap();
        assert_eq!(diff.users_to_update.len(), 1);
        assert!(!diff.users_to_update[0].1.enabled);
        assert!(diff.users_to_update[0].1.secret_access_key == "existing-dummy-secret");
        yaml.users[0].secret_access_key = "rotated-dummy-secret".into();
        let diff = diff_iam(&yaml, &current).unwrap();
        assert_eq!(diff.users_to_update.len(), 1);
        assert!(diff.users_to_update[0].1.secret_access_key == "rotated-dummy-secret");
    }
